...
from core import GiyotinIslem, isleri_yukle, log_event

try:
    from serial_bridge import NanoGiyotin, list_serial_ports
...
def _available_ports_text() -> str:
    ...
    return ", ".join(ports) if ports else "Yok"


def main() -> int:
    print(f"=== GIYOTIN KONTROL (NANO / {PORT}) ===\n")

    if SERIAL_IMPORT_ERROR is not None:
        print("HATA: 'pyserial' bagimliligi yuklu degil.")
        print("Detay:", SERIAL_IMPORT_ERROR)
        print("Kurulum: python -m pip install -r requirements.txt")
        return 10
    ...
    try:
        ...
        print(f"HATA: Nano baglanamadi ({PORT}).")
        print("Detay:", exc)
        print("Mevcut COM portlari:", _available_ports_text())
        return 11
    ...
    try:
        try:
            ...
            print("HATA: Nano'ya komut gonderilemedi.")
            print("Detay:", exc)
            print("Mevcut COM portlari:", _available_ports_text())
            nano.close()
            return 12
        ...
        try:
            while not islem.bitti_mi():
                ...
                if line == "BTN_PRESS":
                    if last_cut_up_t is not None:
                        ...
                        print(
                            f"Operator suresi: {op_time:.2f}s "
                        ...
                        log_event(
                            ...
                            islem.toplam_kesim,
                            extra=(
                                f"operator_sec={op_time:.3f};avg_op_sec={avg_op:.3f};"
                                f"min_op_sec={mn_op:.3f};max_op_sec={mx_op:.3f}"
                            ),
                        )
                    ...
                    if dur is not None:
                        ...
                        print(f"Kesim suresi: {dur:.2f}s | ort: {avg:.2f}s")
            ...
            log_event(
                ...
                islem.mevcut_kesim,
                islem.toplam_kesim,
                extra=f"elapsed_sec={elapsed:.3f}",
            )
            ...
            if cut_durations:
                ...
                print(
                    f"Kesim istatistikleri: ort={avg:.2f}s | min={mn:.2f}s | "
                    f"max={mx:.2f}s | adet={len(cut_durations)}"
                )
